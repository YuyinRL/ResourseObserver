...
public record ObserverDataPayload(
    ...
    List<CraftingBindingSnapshot> craftingBindings // 0.4.0+：AE2 合成快照
)
```

**`craftingBindings`（0.4.0+）** 对每个 `AE2_ITEMS` 绑定提供：

- `CraftableEntry(itemId, displayName, stacked)` —— 可合成物品（上限由服务端决定）
- `CraftingJobEntry(cpuName, busy, outputItemId, totalAmount, remainingAmount, coProcessors, storageBytes)` —— 活跃 CPU 任务
- `CraftingStorageMetrics(cpuCount, busyCpuCount, totalStorageBytes, totalCoProcessors, reliable)` —— 合成存储汇总
