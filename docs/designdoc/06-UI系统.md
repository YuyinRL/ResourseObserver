...
2. **Vanilla Screen V2**（F8 快捷键）— 使用 Minecraft 原生 `Screen` + `GuiGraphics` API，
   组件化的 V2 Panel 架构（`client/screen/v2/`）

两套实现共享同一份**服务端权威 Snapshot**（`service/snapshot/*Builder`），通过
`LatestSnapshotProvider` 静态桥转发最新 payload / snapshot：ModernUI 在
`ViewModelBridge.applyPayload(...)` 同侧发布，V2 Screen 直读最近快照渲染。

V2 Screen 通过 `tick()` 每 20 tick（1s，与 ModernUI `REFRESH_INTERVAL_NORMAL_MS`
对齐）发送 `ObserverRefreshRequestPayload` 主动拉取数据，确保两套 UI 刷新频率一致。
...
| `setSearchQuery(query)` | 更新共享搜索词并重建 Overview / Storage / Crafting ViewModel |
| `setSearchBoxFocused(focused)` | 记录搜索框焦点状态，供页面重建后恢复输入焦点 |
...
顶部：子 Tab 切换栏（`Items` / `Crafting`，0.4.0+）；两个子页共享 KPI 前置区。

- **Items 子 Tab**：KPI 卡片 → 节点列表 → 用量摘要 → 告警筛选 → 物品表格
- **Crafting 子 Tab**（0.4.0+，由 `CraftingSubTabBuilder` 构建）：
  - 顶部 12% 高度存储汇总 KPI 条（CPU 数 / 忙碌 CPU / 可用字节 / 协处理器）
  - 下方 33/67 两列布局：左 Active Jobs 列表（图标 + 进度条 + CPU 名） | 右 搜索 + Craftables 表格（最多 120 条 + "+N more" 提示）
  - 搜索框通过 `ResourceTerminalFragment.scheduleSearchUpdate()` 做 200ms 防抖，本地重建后按 `ViewModelBridge.searchBoxFocused` 自动恢复焦点与光标位置
  - Craftables / 合成树节点支持 `fluid:<namespace>:<path>` 流体键，沿用现有图标与名称解析链路
- 子 Tab 切换保存在 `ViewModelBridge.setStorageSubTab(String)`，通过 `terminal.rebuildContent()` 触发完整重建（Crafting 模式下 `tryIncrementalUpdate` 短路）
...
| `ChartRenderer` | 图表渲染主流程（向量管线） |
| `client.ui.render.chart.*` | 图表公开类型：`ChartPage` / `LineMode` / `SmoothingMode` / `ChartDataType` / `ChartSeriesType` / `ChartHoverPoint` / `RenderResult` |
